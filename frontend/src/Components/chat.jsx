...
const ChatComponent = ({ chatRoomId, unreadMessages, setUnreadMessages }) => {
  ...
  return () => {
    socket.emit("leaveRoom", { room: chatRoomId });
    socket.disconnect();
  };
}, [chatRoomId]);
...
//     const response = await axios.post(`${BASE_URL}/api/chats/updateNoti/${newMessageRoomId}`, { messageId });
  ...
  const fetchMessages = async () => {
    try {
      const response = await axios.get(
        `${BASE_URL}/api/chats/chatMessages/${chatRoomId}`,
        {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const { data } = await axios.post(
        `${BASE_URL}/api/chats/chatSend/${chatRoomId}`,
        {
          content: message,
        },
        {
...
useEffect(() => {
  if (chatRoomId) {
    axios.get(`${BASE_URL}/api/chats/participants/${chatRoomId}`, {
      withCredentials: true
    })
      .then(response => {
...
const typingHandler = (e) => {
  ...
  return (
    <div className='chatPageContainer'  style={{ maxHeight: "400px", position: "relative" }}>
      ...
      <Image style={{width:"43%",height:"80%"}} src={`${BASE_URL}/api/users/uploads/${selectedChat.profileImage}`} alt="Profile" className="followingImage" roundedCircle />
    ) : (
