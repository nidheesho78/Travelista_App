...
const OtherUserFollowersList = () => {
  ...
  useEffect(() => {
    async function fetchFollowersList() {
      try {
        const response = await axios.get(`${BASE_URL}/api/users/otherUserfollowersList/${OtherUserId}`)
      ...
      <div className='listContainer'>
        {filteredFollowingUsers.map(user => {
          const userImage = user.profileImage
            ? `${BASE_URL}/api/users/uploads/${user.profileImage}`
...
export default OtherUserFollowersList;
