...
const AdminReportList = () => {
  ...
  const fetchReportedBlogs = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/admin/reportedBlogs`,{
...
const deleteBlog = async (blogId) => {
    ...
    if (confirmDelete) {
      try {
        const response = await axios.delete(`${BASE_URL}/api/admin/deleteBlog/${blogId}`,{
          withCredentials: true
        });
...
{reportedBlogs.map((blog) => ( 
    <div className='reportBox' key={blog._id}>
      <h4 style={{ fontSize: '1rem'}}>{blog.reportReason}</h4>
      <button className='buttonReportBoxView' onClick={() => deleteBlog(blog._id)}>Delete Blog</button>
